using Intercepts
using Random
using DrWatson
using ProjectRoot
using JLD2

# 2D sweep of class imbalance (μ₀) × regularization (λ/λ_max) for the three
# intercept strategies, on a fixed standardized imbalanced logistic design.
# Records pass counts to a fixed relative-suboptimality tolerance per configuration, so
# downstream code in intercepts.qmd can build the pass-count-ratio heatmap
# that tests the L₀/H₀₀ scaling prediction of @cor-rate-gap and
# @lem-gradient-partial across the (μ₀, λ) plane.

param_dict = Dict{String, Any}(
    "n" => [500],
    "p" => [1000],
    "s" => [10],
    "ρ" => [0.6],
    "μ0" => [0.5, 0.7, 0.9, 0.95, 0.99],
    "reg" => [0.5, 0.2, 0.1, 0.05, 0.02, 0.01],
    "strategy" => [:gradient, :newton, :exact],
    "seed" => [1, 2, 3],
)

params = dict_list(param_dict)

const MAXIT = 5000
const TOL = 1.0e-8

# Checkpoint the expensive grid so an isolated saturated cell cannot discard
# the completed configurations.
const CELLDIR = @projectroot("results", "sim-mu-reg-heatmap", "cells")
mkpath(CELLDIR)

cellpath(μ0, reg, strategy, seed) = joinpath(
    CELLDIR,
    "mu0=$(μ0)_reg=$(reg)_strategy=$(strategy)_seed=$(seed).jld2",
)

results = Vector{Dict{String, Any}}()

for (i, d) in enumerate(params)
    @unpack n, p, s, ρ, μ0, reg, strategy, seed = d
    cached = cellpath(μ0, reg, strategy, seed)
    if isfile(cached)
        record = JLD2.load(cached)["record"]
        if haskey(record, "primals") && haskey(record, "duals")
            push!(results, record)
            println(
                "[$i/$(length(params))] μ0=$μ0 reg=$reg strategy=$strategy seed=$seed (cached)",
            )
            continue
        end
    end

    Random.seed!(seed)

    res = simulated_experiment(
        n,
        p;
        response = :binomial,
        strategy = strategy,
        μ0 = μ0,
        s = s,
        reg = reg,
        ρ = ρ,
        randomize = false,
        maxit = MAXIT,
    )

    d_exp = Dict{String, Any}(copy(d))
    d_exp["strategy"] = String(strategy)
    d_exp["primals"] = res.primals
    d_exp["duals"] = res.duals
    d_exp["gaps"] = res.gaps
    d_exp["relgaps"] = res.relgaps
    d_exp["wall_time"] = res.time[end]

    println(
        "[$i/$(length(params))] μ0=$μ0 reg=$reg strategy=$strategy seed=$seed " *
            "passes=$(length(res.relgaps)) final_dual_relgap=$(res.relgaps[end])",
    )

    jldsave(cached; record = d_exp)
    push!(results, d_exp)
end


suboptimality_against_shared_dual!(
    results;
    instance_of = r -> (r["μ0"], r["reg"], r["seed"]),
)

for r in results
    passes_to_tol = findfirst(g -> g <= TOL, r["relgaps"])
    r["reached_tol"] = passes_to_tol !== nothing
    r["passes"] = isnothing(passes_to_tol) ? length(r["relgaps"]) : passes_to_tol - 1
    r["final_relgap"] = max(r["relgaps"][end], 1.0e-20)

    # The per-cell checkpoints retain the full histories needed to reproduce
    # the shared reference; the assembled artifact only needs plot summaries.
    for key in (
            "primals",
            "duals",
            "gaps",
            "relgaps",
            "dual_relgaps",
            "primal_relgaps",
        )
        delete!(r, key)
    end
end

outfile = @projectroot("results", "sim-mu-reg-heatmap.jld2")
@save outfile results
println("Saved results to $outfile")
