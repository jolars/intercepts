...
def run_sweep(probdir, problem):
    ...
    liblinear_rows = []
    print(f"=== LIBLINEAR (newGLMNET via scikit-learn) problem={problem} ===")
    ...
    blitz_rows = []
    print(f"=== BlitzL1 problem={problem} ===")
    ...
    return pd.DataFrame(liblinear_rows + blitz_rows)
...
out_synth.to_csv(synthetic_dir / "proxnewton.csv", index=False)
print(f"Wrote {synthetic_dir / 'proxnewton.csv'}")

out_w1a = run_sweep(w1a_dir, "w1a")
out_w1a.to_csv(w1a_dir / "proxnewton.csv", index=False)
print(f"Wrote {w1a_dir / 'proxnewton.csv'}")

out_news20 = run_sweep(news20_dir, "news20-3pct")
out_news20.to_csv(news20_dir / "proxnewton.csv", index=False)
print(f"Wrote {news20_dir / 'proxnewton.csv'}")
