...
function load_problem(name::String;
    ...
    pos = maximum(y); y = Int.(y .== pos)

    if n_positive !== nothing || n_negative !== nothing
        rng = MersenneTwister(seed)
        pos_idx = findall(==(1), y); neg_idx = findall(==(0), y)
        ...
        keep_rows = sort(vcat(shuffle(rng, pos_idx)[1:n_pos_keep],
...
import Intercepts: LogisticLoss, residual, gradient, hessian, loss, lambdamax,
                   ...
                   ExactStrategy, rescalecoefs

function run_instrumented(X, y, reg; randomize::Bool, maxit::Int, maxtime::Float64,
                          tol::Real = 1e-10)
    ...
    while it < maxit && (time() - t0) < maxtime
        ...
        grad_all = vec(x' * θ)
        if sparse_norm
            θsum = sum(θ); grad_all .-= x_sparse_offset .* θsum
        end
        ...
        push!(primal_pre_history, primal); push!(relgap_history, relgap)
        push!(intercept_history, intercept)

        # intercept-axis gradient before the pass
        η_grad_pre = gradient(lossfun, η, y); grad0_pre = sum(η_grad_pre)
        ...
        for (inner_it, j) in enumerate(ind)
            ...
            if inner_it % update_when == 0
                ...
                intercept, k0 = update_intercept_with_count(intercept_strategy,
        ...
        η_grad_post = gradient(lossfun, η, y); grad0_post = sum(η_grad_post)
...
n, p = size(X_w1a)
@printf("w1a: n=%d, p=%d, %% positive = %.3f, density = %.4f\n",
        n, p, mean(y_w1a), nnz(X_w1a) / (n*p))

println("\n=== w1a + cyclic + exact ===")
res_w1a_cyc = run_instrumented(X_w1a, y_w1a, 0.05;
                               randomize = false, maxit = 60, maxtime = 60.0)
...
for k in 1:length(res_w1a_cyc.relgap_history)
    ...
    g0p = k <= length(res_w1a_cyc.grad0_pre_history) ? res_w1a_cyc.grad0_pre_history[k]  : NaN
    g0q = k <= length(res_w1a_cyc.grad0_post_history) ? res_w1a_cyc.grad0_post_history[k] : NaN
    dr  = k <= length(res_w1a_cyc.drift_pass_history) ? res_w1a_cyc.drift_pass_history[k] : NaN
    α   = k <= length(res_w1a_cyc.alpha_pass_history) ? res_w1a_cyc.alpha_pass_history[k] : NaN
    dp  = (k <= length(res_w1a_cyc.primal_post_history)) ?
          (res_w1a_cyc.primal_post_history[k] - res_w1a_cyc.primal_pre_history[k]) : NaN
...
println("\n=== w1a + permuted + exact ===")
Random.seed!(42)
res_w1a_perm = run_instrumented(X_w1a, y_w1a, 0.05;
                                randomize = true, maxit = 60, maxtime = 60.0)
...
for k in 1:length(res_w1a_perm.relgap_history)
    ...
    α  = k <= length(res_w1a_perm.alpha_pass_history) ? res_w1a_perm.alpha_pass_history[k] : NaN
    dr = k <= length(res_w1a_perm.drift_pass_history) ? res_w1a_perm.drift_pass_history[k] : NaN
