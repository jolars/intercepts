...
import Intercepts: LogisticLoss, residual, gradient, hessian, loss, lambdamax,
                   normalizefeatures, st, update_intercept_with_count, ExactStrategy

function run_no_ls(X, y, reg; randomize::Bool, maxit::Int = 200)
    lossfun = LogisticLoss(); intercept_strategy = ExactStrategy()
    n, p = size(X)
    ...
    for it = 1:maxit
        ...
        push!(relgap_history, relgap); push!(primal_history, primal)
        relgap <= 1e-10 && break
...
@printf("passes=%d  final relgap=%.3e  final primal=%.6f\n",
        length(res.relgap_history), res.relgap_history[end], res.primal_history[end])
@printf("min relgap during run: %.3e at pass %d\n",
        minimum(res.relgap_history), argmin(res.relgap_history))
