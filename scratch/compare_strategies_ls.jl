...
import Intercepts: LogisticLoss, residual, gradient, hessian, loss, lambdamax,
                   ...
                   NewtonStrategy, GradientStrategy, ExactStrategy

function run_strat(X, y, reg; strategy, randomize::Bool, maxit::Int = 200,
    ...
    relgap_history = Float64[]; alpha_history = Float64[]
    t0 = time()
    for it = 1:maxit
        ...
        grad_all = vec(x' * θ)
        if sparse_norm; grad_all .-= x_sparse_offset .* sum(θ); end
        ds = max(1, norm(grad_all, Inf) / λ); θ ./= ds
        ...
        for (inner_it, j) in enumerate(ind)
            coef_j = coef[j]
            ηg = gradient(lossfun, η, y); ηh = hessian(lossfun, η, y)
            g = dot(view(x,:,j), ηg)
            ...
            if diff != 0
                η .-= diff .* view(x,:,j)
                if sparse_norm; η .+= diff * x_sparse_offset[j]; end
            end
            if inner_it == p
                ip = intercept
        ...
        cdiff = coef - coef_old; idiff = intercept - intercept_old
        α = 1.0
        while primal_after > primal && α > 1e-4
            α *= 0.1
            coef = coef_old + α * cdiff
            intercept = intercept_old + α * idiff
            η = x * coef .+ intercept
            if sparse_norm; η .-= dot(x_sparse_offset, coef); end
            primal_after = loss(lossfun, η, y) + λ * norm(coef, 1)
        end
...
for (name, strat) in [
    ...
    res = run_strat(Xw, yw, 0.05; strategy = strat, randomize = false,
                    maxit = 1000, maxtime = 30.0)
    n_low = count(<(1e-3), res.alpha_history)
    @printf("strategy=%-8s passes=%4d  final_relgap=%.3e  passes_with_α<1e-3=%d\n",
            name, length(res.relgap_history), res.relgap_history[end], n_low)
    if n_low > 0
        # find first low-alpha pass
        k = findfirst(<(1e-3), res.alpha_history)
