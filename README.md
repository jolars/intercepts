...
inputs; “none” means that the producer generates its own synthetic data.
  ...
  | `@tbl-multinomial-counts`; class-count diagnostics for the multinomial sweep | `results/sim-multinomial-prevalence.csv` | `julia --project=. experiments/sim-multinomial-prevalence.jl` (reads the committed baseline and sweep caches) | Julia | None |
