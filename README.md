...
This is a project about how to fit intercepts for generalized linear models
(GLMs) when using coordinate descent.

## Project structure

This project is structure for the
[Compute journal](https://computo.sfds.asso.fr/), a journal for reproducible
research in computational science. It is built around a
[Quarto](https://quarto.org/) notebook, which is a document format that allows
you to combine text, code, and results in a single document. But the project
also contains some code to run experiments and generate results for the
notebook:
...
This project is built using Julia 1.11, which you can install through juliaup, a
tool to manage Julia installations.
...
the <scripts/> directory. For example, to reproduce the results for the
...
If you are using [NixOS](https://nixos.org/) or the
[Nix package manager](https://nixos.org/download.html), then we have provided a
`flake.nix` file that can be used to reproduce the environment for the project.
