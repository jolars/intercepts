...
numerically. [Supplement S3](#sec-s-methodology) documents its tolerances,
iteration cap, and scalar floating-point margin; a completed experiment
therefore distinguishes a numerically certified conditional solve from an
uncontrolled capped approximation.
...
- We use the Schur complement to isolate the curvature ratio $H_{00}/L_0$ that
  ...
  binary logistic regression, this ratio diverges as the positive-class marginal
  $\mu_0$ approaches one.
...
gradient strategy uses the global bound $L_0$, Newton uses the local curvature
$H_{00}$, and exact minimizes the intercept subproblem. On imbalanced data,
$H_{00}$ can be much smaller than $L_0$. When the intercept---or a direction
coupling the intercept and a coefficient---controls convergence, this mismatch
can make the gradient strategy slower than Newton by a factor proportional to
$L_0/H_{00}$. Below, we distinguish the local result that we prove from a
bound-based heuristic and the behavior that we observe experimentally.
...
Poisson models. A later section extends them to multinomial logistic regression,
whose intercept is a vector. For observation $i$, let
$\eta_i = x_i^\intercal \beta + \beta_0$ denote the linear predictor, and let
...
pass is descent by construction; the pseudocode omits this guard for
