...
[^cold-start-footnote]: At the single-$\lambda$ configuration reported in
    @sec-cold-start-diag, biglasso `alg.logistic = "Newton"` returns inaccurate
    fits on both extreme-imbalance real datasets, while adelie returns an empty
    state on w1a. These results use loose solver tolerances and do not describe
    either package's default configuration.
