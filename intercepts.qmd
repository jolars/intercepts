...
:   Iterates Armijo-safeguarded Newton steps on the intercept subproblem until
    the normalized intercept gradient is at most $10^{-10}$ for scalar models
    and $10^{-8}$ for multinomial blocks. The implementation raises an error
    rather than returning an unconverged iterate if it reaches its 50-step inner
    limit.

The analysis uses *exact* for the ideal conditional minimizer. The empirical
strategy is its numerical realization with the residual guarantees above; a
completed experiment therefore distinguishes a tolerance-certified conditional
solve from a capped approximation.
...
3. Repeated safeguarded Newton steps on the intercept subproblem, iterated until
   the normalized intercept gradient is at most $10^{-10}$---the *exact
   strategy*.
...
Yeoh2002, the gradient strategy needs about $3.8$ times as many passes as Newton
to bring the certified upper bound below $10^{-4}$. On Shuttle, whose rarest
classes contain only ten and thirteen observations, it fails to reach $10^{-4}$
within 1000 passes; Newton and exact reach that threshold in about 25 passes.

```{julia}
#| label: fig-multinomial-summary
#| fig-cap: Multinomial logistic regression on two real datasets. The panels
#|   show a certified relative suboptimality upper bound versus time for
#|   Yeoh2002 ($p \gg n$) and StatLog Shuttle ($n \gg p$) at $\lambda =
#|   0.05\lambda_{\max}$ under cyclic coordinate descent.
...
draw(
    visual(Lines) *
        ...
        mapping(
            :time => "Time (s)",
            :relgaps => "Certified relative suboptimality upper bound",
            color = :strategy,
...
$1.17 \times$--$1.37 \times$ as many inner work units while finishing in
...
convergence tolerance and plot a certified upper bound on suboptimality against
its available measure of work (CD passes, outer iterations, or wall-clock time).
...
$P - D^\star$, a certified upper bound on each run's suboptimality. This bound
cannot claim more precision than the shared certificate, unlike the point
estimate $P - F^\star$.
...
#|   panels show a certified suboptimality upper bound versus inner CD passes
#|   for `type.logistic = "Newton"` and `"modified.Newton"` in path mode on the
#|   shared synthetic problem, w1a, and news20-3pct. All modes use the common
#|   feasible dual reference $D^\star$ for each problem.
...
draw(
    ...
    axis = (
        ...
        ylabel = "Certified suboptimality upper bound",
    ),
    figure = (; size = (800, 300)),
)
```
...
#|   panels show a certified suboptimality upper bound versus inner CD passes
#|   for `alg.logistic = "Newton"` and `"MM"` in path mode on the same three
#|   problems as @fig-real-glmnet. All modes use the common feasible dual
...
#|   certified suboptimality upper bound versus `AndersonCD` outer iterations on
#|   the same three problems as @fig-real-glmnet, using the common feasible dual
#|   reference $D^\star$ for each problem.
...
#|   panels show a certified suboptimality upper bound versus wall-clock time
#|   for LIBLINEAR and BlitzL1 on the same three problems as @fig-real-glmnet.
#|   LIBLINEAR runs that hit its internal `max_iter` cap without further
#|   progress are omitted. Both solvers use the common feasible dual reference
...
reference certificate: the certified upper bounds reach approximately
...
#|   certified suboptimality upper bound versus convergence tolerance on the
#|   same three problems as @fig-real-glmnet. The horizontal axis is the coupled
#|   outer/inner tolerance (`tol = irls_tol`), and the bound uses the common
#|   feasible dual reference $D^\star$ for each problem.
...
#|   certified relative suboptimality upper bound versus time for $K = 5$, class
#|   marginals $(0.7, 0.1, 0.1, 0.05, 0.05)$, $n = 500$, $p = 200$, $\lambda =
...
draw(
    ...
    axis = (
        yscale = log10,
        ylabel = "Certified relative suboptimality upper bound",
        xlabel = "Time (s)",
    ),
    facet = (; linkxaxes = :none),
    figure = (; size = (800, 250)),
...
We report a certified relative upper bound on suboptimality
(@fig-multinomial-imbalance). The multinomial CD solver constructs a feasible
dual point by centering and rescaling the residual onto the $\ell_\infty$ ball
...
#| fig-cap: Multinomial imbalance-amplitude sweep. The panels show a certified
#|   relative suboptimality upper bound versus outer-pass index for the four
#|   corners of the rarest-class marginal $\bar p_K \in \{0.05, 0.005\}$ and
#|   feature-amplitude $\in \{0.5, 2.0\}$ grid on the same $K = 5$, $n = 500$,
#|   $p = 200$, $s = 5$, $\lambda = 0.05\lambda_{\max}$, $\rho = 0.3$ design as
...
#|   certified relative suboptimality upper bound versus time at $\lambda =
#|   0.05\lambda_{\max}$ under cyclic CD for $n = 248$, $p = 12\,625$, and $K =
#|   6$. Suboptimality is bounded using the strongest shared feasible dual
#|   point.
...
pattern. The slowdown ratio ($\approx 3.8 \times$ at certified upper bound
...
#|   certified relative suboptimality upper bound versus time at $\lambda =
#|   0.05\lambda_{\max}$ under cyclic CD for $n = 58\,000$, $p = 9$, and $K =
#|   7$. Suboptimality is bounded using the strongest shared feasible dual
#|   point.
...
On Shuttle, the Newton and exact strategies bring the certified upper bound
below $10^{-4}$ in $25$ and $23$ passes respectively, while the gradient
...
median certified relative suboptimality upper bound over three seeds.

```{julia}
#| label: fig-multinomial-sweep-full
#| fig-cap: Full multinomial imbalance-amplitude sweep. The panels show a
#|   certified relative suboptimality upper bound versus outer-pass index over a
#|   $4 \times 4$ grid of rarest-class marginal $\bar p_K \in \{0.05, 0.02,
#|   0.01, 0.005\}$ and feature amplitude $\in \{0.5, 1.0, 1.5, 2.0\}$. Curves
#|   are medians over three seeds for $K = 5$, $n = 500$, $p = 200$, $s = 5$,
#|   $\rho = 0.3$, and $\lambda = 0.05\lambda_{\max}$.
...
approaches the joint optimum. Summed across the run, the exact-to-Newton work
ratios are $1.17$, $1.25$, and $1.37$ for $\mu_0 = 0.5, 0.9, 0.99$,
respectively. The trend is monotone in the imbalance that drives
...
109/108/121; exact: 109/108/120). The exact strategy spends more inner work as
imbalance grows without improving outer-pass progress (@fig-per-pass-progress).
...
satisfies the stated conditional-gradient tolerance; reaching the cap would
raise an error rather than produce a cached result. This behavior is consistent
with local quadratic convergence after the iterates approach their conditional
minimizers, but the experiment, rather than a global Newton bound, establishes
the reported cost. The exact strategy incurs most of its overhead in the first
few cold-start passes, when $|\partial_0 F|$ is largest. The warm-start regime
of [Supplement S4.2](#sec-warmstart-path) largely avoids that window.
...
#|   certified suboptimality upper bound versus wall-clock time for glmnet,
#|   adelie, and skglm `ProxNewton` in path mode at $n = 529$, $p = 999$, $\bar
#|   y = 11.83$, and $\lambda = 0.05\lambda_{\max}$. Every solver uses the
#|   common certified feasible dual reference $D^\star$.
