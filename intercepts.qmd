...
floating-point margin in [Supplement S3](#sec-s-methodology); a completed
experiment therefore distinguishes a numerically certified conditional solve
from an uncontrolled capped approximation.
...
readability, and we state its constants in [Supplement S3](#sec-s-methodology).
...
improvement in the outer trajectory. We give the full block derivation, the
relevant Rayleigh quotients, and the synthetic sweeps in [Supplement
S2.1](#sec-vector-intercepts).
...
iterate. We use pinned datasets in the real-data experiments and follow the same
shared-reference principle in the production comparison. Cached outputs support
the expensive sweeps; the toy example in @fig-parametric runs live when the
notebook renders. We pin the Julia, R, and Python environments in the repository
and give the complete construction, tolerances, budgets, data preparation,
hardware, and reproduction protocol in [Supplement S3](#sec-s-methodology).
...
respective evaluation points. We examine this discrepancy over a wider grid in
@sec-imbalance-reg.
...
trajectories overlap in every one of the sixteen cells. In fourteen cells, their
median suboptimality bounds reach at most $1.3 \times 10^{-7}$, while the
gradient strategy generally slows as $\bar p_K$ shrinks and amplitude grows. In
the two most extreme cells ($\bar p_K = 0.005$ and amplitude at least $1.5$), no
