...
- We extend the analysis to vector intercepts in multinomial logistic
  ...
  regimes: the Yeoh2002 pediatric ALL gene-expression panel ($K = 6$, $p \gg n$,
  rarest class $6\%$) and the StatLog Shuttle sensor-data dataset ($K = 7$,
  $n \gg p$, rarest class $0.02\%$; @fig-multinomial-summary). On both, the
  gradient strategy stalls in low-curvature modes associated with rare classes,
  while the Newton and exact strategies follow nearly the same trajectory.
...
Yeoh2002, the gradient strategy needs about three times as many passes as Newton
to reach relative suboptimality $10^{-6}$. On Shuttle, whose rarest classes
contain only ten and thirteen observations, it fails to reach $10^{-4}$ within
1000 passes; Newton and exact reach $10^{-6}$ in about 30 passes.

```{julia}
#| label: fig-multinomial-summary
#| fig-cap: Multinomial logistic regression on two real datasets. The panels
#|   show relative primal suboptimality versus time for Yeoh2002 ($p \gg n$) and
#|   StatLog Shuttle ($n \gg p$) at $\lambda = 0.05\lambda_{\max}$ under cyclic
#|   coordinate descent.
...
$\rho = 0.6$. Real-data experiments use pinned datasets and the production
comparison evaluates every solver against a shared, independently certified
reference objective. Cached outputs support the expensive sweeps; the toy
example in @fig-parametric runs live when the notebook renders. The repository
pins the Julia, R, and Python environments. [Supplement S3](#sec-s-methodology)
gives the complete tolerances, budgets, data preparation, hardware, and
...
two methods still differ in their inner work, measured directly in [Supplement
S4.3](#sec-per-pass-cost).
...
permuting the coordinates changes pass counts by at most a factor of $1.4$;
...
$1.30 \times$--$1.59 \times$ as many inner Newton steps while finishing in
essentially the same number of outer passes. Most of that overhead occurs in the
first cold-start passes; warm starts largely avoid it ([Supplement
S4.3](#sec-per-pass-cost)). The full ordering, path, and per-pass trajectories
appear in [Supplement S4](#sec-s-robustness).
...
overhead, as [Supplement S4.3](#sec-per-pass-cost) shows.
...
The curvature ratio $H_{00}/L_0$ explains this rule. A global-majorant update
removes only that fraction of the intercept--coefficient coupling
(@eq-grad-residual), whereas Newton uses the local curvature. The frozen model
then explains how this one-step mismatch becomes the late-stage $L_0/H_{00}$
rate plateau under strong collective coupling (@prp-frozen-block-rate,
@fig-rho-frozen). This is a local explanation, not a global iteration-complexity
theorem.

The production comparisons test the rule outside our implementation. The most
informative evidence comes from interventions that hold the surrounding solver
fixed: the weight-mode switches in glmnet and biglasso and the before-and-after
skglm comparison (@sec-production-solvers). Their agreement with the internal
experiments supports curvature choice, rather than package identity, as the
...
median relative primal suboptimality over three seeds.

```{julia}
#| label: fig-multinomial-sweep-full
#| fig-cap: Full multinomial imbalance-amplitude sweep. The panels show relative
#|   primal suboptimality versus outer-pass index over a $4 \times 4$ grid of
#|   rarest-class marginal $\bar p_K \in \{0.05, 0.02, 0.01, 0.005\}$ and
#|   feature amplitude $\in \{0.5, 1.0, 1.5, 2.0\}$. Curves are medians over
...
The full grid shows a smooth strengthening of the effect seen at the four
corners. The gradient strategy stalls more severely as the rarest class becomes
rarer and as the feature amplitude grows. Newton and exact remain nearly
indistinguishable in every cell. The corner panels in @fig-multinomial-sweep
therefore summarize a systematic pattern rather than a favorable selection from
the grid.
...
warm-start regime of [Supplement S4.2](#sec-warmstart-path) largely avoids that
window.
...
from the preceding solution and runs to relative-gap tolerance $10^{-8}$, a
budget of 300 outer passes, or a 20-second time limit.
...
comparison among intercept strategies. Warm starts bring the Newton and exact
curves close together under both orderings and reduce their budget-limited
subproblems on w1a. The gradient strategy remains largely insensitive to warm
starting on w1a, where its small intercept correction controls the rate; on the
less imbalanced simulated problem, all three strategies remain competitive.
