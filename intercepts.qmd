...
is_pdf = output_format == "latex"

function output_size(size)
    width, height = size
    scale = min(580 / 800, 580 / width)
    return is_pdf ? round.(Int, scale .* size) : size
end

set_theme!(theme_latexfonts(); fontsize = is_pdf ? 10 : 14, size = output_size((800, 450)))
...
fig = Figure(; size = output_size((440, 300)))
...
draw(
    ...
    figure = (; size = output_size((430, 280))),
)
```
...
(@tbl-classification). In binary logistic regression, the scalar Schur-coupling
residual quantifies the cost: when the response is imbalanced, $H_{00}/L_0$
shrinks to zero, the MM intercept step shrinks in direct proportion to the local
Newton step, and its effect on the next coefficient gradient is attenuated to
first order (@eq-grad-residual). Its multinomial analogue replaces that scalar
ratio with the eigenvalues of $H_{00}/L_0$ (@eq-grad-residual-matrix). The MM
literature treats this as loose majorization. We show that the bound is
routinely loose for imbalanced binary logistic and rare-class multinomial
problems (@sec-irls, @fig-multinomial-sweep).
...
By separability, write
$G_\lambda(\beta) = \sum_{j = 1}^p g_{\lambda,j}(\beta_j)$ and define
$$
  \operatorname{prox}_{\gamma g}(v) = \argmin_{u \in \mathbb{R}} \left\{g(u) + \frac{1}{2\gamma}(u - v)^2\right\}.
$$
The coordinate update is therefore the proximal-gradient step
$$
  \beta_j^+ = \operatorname{prox}_{\gamma_j g_{\lambda,j}} \left(\beta_j - \gamma_j \frac{\partial}{\partial\beta_j}F(\beta_0,\beta)\right).
$$
...
In the implementation, every coefficient update in @algo-cd is additionally
wrapped in a Tseng--Yun Armijo backtrack on the proximal model decrease, so each
pass is descent by construction; the pseudocode suppresses this guard for
readability, and [Supplement S3](#sec-s-methodology) states its constants. The
Newton branch of @algo-cd wraps the same Newton direction in an Armijo
...
Across the experimental imbalance sweep, $k_0 \in [1, 5]$, and each step costs
...
draw(
    ...
    figure = (; size = output_size((800, 280))),
)
```
...
further $1/L_0$ steps on the original loss. The correction transmitted per
re-linearization is nevertheless governed by the same attenuation, so the
Schur-coupling residual (@eq-grad-residual) applies directly to glmnet's
...
(`Newton` versus `MM`) isolate the weight scheme in @eq-grad-residual.
...
$10^{-8}$ in 24--26 passes across the three seeds (@fig-irls-comparison).
Direct-CD with Newton needs 49--60 passes, and Direct-CD with the gradient
strategy needs 340--407. MM-IRLS does not reach even $10^{-4}$ in 200 passes;
its final bounds range from $3.1 \times 10^{-4}$ to $6.7 \times 10^{-4}$. Within
...
731 passes to bring the relative suboptimality bound below $10^{-8}$, compared
with 50 passes for Newton and 52 for exact. The latter two lie almost directly
on top of each other across the full sweep (@fig-mu-extreme). This agreement
supports the prediction that exact profiling will add little outer progress over
one Newton step (@eq-intercept-drift). The two methods still differ in their
inner work, measured directly in [Supplement S4.3](#sec-per-pass-cost).
...
strategies remain close across the sweep (@fig-mu-extreme).
...
draw(
    ...
    axis = (yscale = log10,),
    facet = (; linkxaxes = :none),
    figure = (; size = output_size((900, 500))),
)
```
...
designs is consistent with response imbalance driving the gap rather than a
peculiarity of w1a.
...
draw(spec_grad * visual(Heatmap), figure = (; size = output_size((380, 250))))
```
...
draw(
    ...
    figure = (; size = output_size((380, 250))),
)
```
...
draw(spec_sk * visual(Heatmap), figure = (; size = output_size((380, 250))))
```
...
draw(
    ...
    figure = (; size = output_size((580, 300))),
    facet = (; linkxaxes = false),
)
```
...
Neither curve uniformly dominates: the cold-start curve is tighter at
$\mu_0 = 0.5$ and the iterate-level curve is tighter from $\mu_0 = 0.7$ on. The
two bracket the empirical ratio up to $\mu_0 = 0.9$, sit marginally below it at
$\mu_0 = 0.95$, and both sit above it at $\mu_0 = 0.99$, where they approach the
...
#|   dashed $L_0/H_{00}$ reference as the centering fraction $\alpha$ of
#|   @eq-centering-sweep moves the weighted $\bar\rho^2$ from standardized to
#|   uncentered designs for $n = 500$, $p = 1000$, $s = 10$, $\lambda =
#|   0.05\lambda_{\max}$, and permuted coordinate ordering.
...
draw(
    ...
    figure = (; size = output_size((420, 280))),
)
```
...
$\approx 3.7 \times$ pass-count ratio because the rarest Shuttle classes (ten,
...
that mechanism. Quantitatively, at the $10^{-6}$ threshold the median gradient
curve needs $10$--$41$ times as many passes as Newton in the five cells where it
reaches that bound within the $1\,000$-pass budget, and fails to reach it in
nine further cells in which Newton needs at most $158$ passes; the remaining two
cells are the saturated ones just discussed.
...
Real-data experiments use pinned datasets from Zenodo (DOI:
[10.5281/zenodo.20315625](https://doi.org/10.5281/zenodo.20315625)) and
standardize them before fitting. The repository includes the code needed to
retrieve these data and to generate the shared standardized problem used in the
...
Given the cached inputs, the drivers are otherwise deterministic, so each solver
is run once per tolerance setting while sweeping its native convergence
...
applies after every coordinate update ([Supplement S3](#sec-s-methodology)): on
the w1a benchmark of @fig-first-example, all three strategies converge under
both orderings and the pass-count differences are within a factor of $1.9$
...
draw(
    visual(Lines) * spec,
    axis = (; yscale = log10),
    figure = (; size = output_size((500, 240))),
)
```
...
draw(
    ...
    axis = (yscale = log10, xlabel = "Time (s)", ylabel = "Suboptimality"),
    figure = (; size = output_size((500, 280))),
)
```
