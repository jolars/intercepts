...
approaching a true stall only as fitted probabilities reach the boundary. There
is a historical irony here---MM majorization was introduced for numerical
stability of the working response $z_i$ when $w_i \to 0$, which is the same
imbalance regime where the resulting update converges slowly. For Poisson loss
no global upper bound on $f''$ exists, so the MM construction is not directly
available; glmnet uses local-weight IRLS for Poisson, where the three strategies
coincide as described above.
...
by the predicted slowdown $L_0/H_{00} = 1 / (4 \pi_+ (1 - \pi_+))$ (the table
...
They sit at nearly the same $\pi_+$ on very different designs---a $300$-feature
...
bag-of-words. Agreement between them would rule out a quirk specific to w1a.
  ...
  | dataset       | $n$   | $p$   | density | $\pi_+$ | $H_{00}/L_0$ |
  | :------------ | ----: | ----: | :------ | ------: | -----------: |
  ...
  : LIBSVM problems used in @fig-real-logreg. $\pi_+$ is the positive-class
    marginal, and $H_{00}/L_0 = 4\pi_+(1-\pi_+)$. news20-3pct is a deterministic
