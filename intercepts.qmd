...
Equation @eq-grad-residual describes one coefficient update; it does not give a
convergence rate. We next connect that mechanism to the observed slowdown in two
layers. First, @eq-rate-gap gives a deliberately non-sharp scaling heuristic for
cold-start pass counts; @fig-rho-centering then shows that it predicts neither
the transition nor the late-stage plateau. Second, the frozen local model in
@prp-frozen-block-rate and @fig-rho-frozen explains both. The heuristic
motivates the relevant curvature ratio; the local model supplies the rate
explanation.

#### A scaling heuristic and its limit
