...
across different designs support the response-imbalance mechanism, although
the fitted curvature and collective coupling determine the local rate.
...
The two heatmaps support the curvature-mismatch mechanism across penalties:
the empirical ratio generally grows as the intercept update becomes more
...
holding $H_{00}/L_0$ fixed. The frozen coordinate-sweep model in
@fig-rho-frozen accounts for the transition from similar rates to the
$L_0/H_{00}$ plateau as the coupled mode becomes slowest.
...
$H_{00} - H_{0A}H_{AA}^{-1}H_{A0} = H_{00}(1 - \kappa)$, the intercept
curvature after the coefficients have adjusted.
...
$$
This descriptive measure records coupling to individual coefficients;
$\kappa$ also accounts for their joint adjustment through $H_{AA}^{-1}$.
...
In @fig-rho-centering, $T_{\mathrm{G}}$ and $T_{\mathrm{N}}$ count passes to
the shared relative suboptimality bound $10^{-6}$. Their empirical ratio rises
sharply as coupling increases from near zero and then settles near the
strong-coupling limit $L_0/H_{00}$ in @eq-frozen-block-rate. To account for
both the transition and the plateau, we retain the coefficient error modes of
the actual coordinate sweep in the calculation below.
...
#|   uncentered designs. Here $n = 500$, $p = 1000$, $s = 10$,
#|   $\lambda = 0.05\lambda_{\max}$, and the coordinate ordering is permuted.
...
Let $C_\pi$ be the frozen linear map for a coefficient sweep in order $\pi$,
and let
$$
...
changing the weight scheme also changes the coefficient updates. skglm
(version 0.5) applies the gradient strategy directly to the original loss. Its
slow convergence from suboptimality $\approx 10^{-1}$ to $\approx 10^{-5}$ in
