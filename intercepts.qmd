...
$$
  F(\beta_0, \beta)
  = \frac{1}{n}\sum_{i=1}^n f(x_i^\intercal \beta + \beta_0; y_i).
$$
...
Across the imbalance levels considered in our experiments, $k_0 \in [1, 5]$, and
each step costs $O(n)$ work (@fig-per-pass-cost).
...
$$
  H_{00}
  = \frac{1}{n}\sum_{i=1}^n \left\{ \operatorname{diag}(p_{i,1:K-1}) - p_{i,1:K-1}p_{i,1:K-1}^{\intercal} \right\}.
$$
...
$$
  ...
  = \frac{1}{2n} \sum_{i=1}^n w_i^{(t)} \bigl( z_i^{(t)} - \beta_0 - x_i^\intercal \beta \bigr)^2,
$$
...
It does not do so under the per-coordinate Armijo backtracking used by our
coordinate-descent solver, `cdsolver`, after every coordinate update
([Supplement S3](#sec-s-methodology)). On the w1a benchmark of
@fig-first-example, all three strategies converge under both orderings and the
pass-count differences are within a factor of $1.9$
