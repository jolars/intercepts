...
are identical. The exact strategy tracks Newton on every panel, consistent with
@eq-intercept-drift at this tolerance.
...
the exact strategy needs essentially the same number of outer passes as Newton.
Its price is therefore a modest inner-solve overhead that grows with imbalance,
not an outer-pass improvement, as [Supplement S4.3](#sec-per-pass-cost) shows.
...
Warm-starting therefore narrows the gap between Newton and exact: a single
Newton step and the exact strategy become hard to distinguish in practice. It
does not, however, rescue the gradient strategy. The recommendation from
@eq-intercept-drift still stands---a safeguarded Newton step in direct CD and a
fully resolved intercept on the frozen quadratic at a prox-Newton/IRLS boundary.
The exact original-loss strategy is also defensible in a warm-started path
solve.
