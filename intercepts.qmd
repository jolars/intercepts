...
update as a routine implementation choice. But that choice can dominate
convergence speed when the response is imbalanced. For direct coordinate
descent, a single safeguarded Newton step per pass removes this slowdown in our
experiments.
...
We compare three strategies for updating the intercept once per pass over the
coefficient coordinates:

Gradient Strategy
:   Takes a single gradient step based on a global curvature bound.

Newton Strategy
:   Takes a single Newton step based on the curvature at the current iterate,
    with an Armijo backtracking line search to guard against overshooting.

Exact Strategy
:   Minimizes the loss over the intercept while holding the coefficients fixed.

In the analysis, *exact* means setting the intercept to its conditional
minimizer. The experiments compute this minimizer numerically, with tolerances
documented in [Supplement S3](#sec-s-methodology).

These strategies can converge at substantially different rates when the response
is imbalanced---for example, when positive cases are rare in logistic
regression. The three paths in @fig-parametric trace the strategies across the
level sets of @eq-primal-problem for a toy problem with one feature and an
intercept. The gradient strategy approaches the solution through many small
steps, whereas the Newton and exact strategies reach it in only a few.
...
- We use a Schur complement to describe how an intercept update changes the
  coefficient gradients. A frozen quadratic model then explains why, under
  strong coupling, the gradient-to-Newton pass-count ratio approaches the ratio
  of global to local intercept curvature.
...
(`solver_glm_naive.hpp:366`).] For Gaussian data, the framework in @sec-theory
reduces to the familiar result: the loss has constant curvature, so the gradient
and Newton strategies coincide on the intercept.
...
proportional to the ratio between the two. We first analyze the effect of one
intercept update, then follow its effect through repeated passes on a fixed
quadratic model and compare the resulting local rates with experiments.
...
is unpenalized. The resulting coordinate-descent algorithm appears in @algo-cd.
...
$$
  ...
  = \partial_j F(\beta_0, \beta)
    - \alpha H_{j0} \partial_0 F(\beta_0, \beta) + r_j(\alpha),
...
$$ {#eq-newton-coupling}
i.e. the profiled gradient plus a bias $b_j = H_{j0}^*r_0 + O(r_0^2)$. To assess
its effect on the penalized update, compare both coordinate steps at the same
positive curvature $h_j$. Write
$G_\lambda(\beta) = \sum_j G_{\lambda,j}(\beta_j)$ and define the profiled step
$$
  d_j^{\mathrm{prof}}
  = \operatorname{prox}_{G_{\lambda,j}/h_j} \left( \beta_j - \frac{\partial_j\tilde F(\beta)}{h_j} \right)
    - \beta_j.
$$
The step $d_j^{\mathrm{N}}$ after the Newton intercept update replaces
$\partial_j\tilde F$ by $\partial_j\tilde F + b_j$. Nonexpansiveness of the
proximal map gives
$$
  |d_j^{\mathrm{N}} - d_j^{\mathrm{prof}}| \leq \frac{|b_j|}{h_j},
  \qquad
  |b_j| \ll h_j |d_j^{\mathrm{prof}}|
$$ {#eq-newton-good-enough}
as an absolute perturbation bound and a sufficient criterion for a small
relative step error, respectively. The relevant comparison is with the composite
coordinate residual $h_j d_j^{\mathrm{prof}}$. At an active lasso optimum,
$\partial_j\tilde F = -\lambda\operatorname{sign}(\beta_j)$, so the smooth
gradient itself generally does not vanish.

Under the local conditions of @eq-newton-error, $b_j$ is $O(|\partial_0 F|^2)$
when the cross derivatives remain bounded. If $h_j$ stays bounded away from
zero, the profiled coordinate step remains first order in the iterate error, and
$\partial_0 F$ vanishes at least linearly, the Newton perturbation is second
order and becomes negligible relative to that step. This is a conditional local
comparison: when the profiled step is zero or vanishes faster, the absolute
bound in @eq-newton-good-enough still applies, but it supplies no relative
guarantee.

Far from the optimum, the full-step approximation can fail when the third
derivative is large, the intercept curvature approaches zero, or the intercept
gradient is large. Poisson regression, where $f''' = e^\eta$, can exhibit this
cold-start behavior. Armijo backtracking safeguards descent, but its accuracy
must be distinguished from that of a full Newton step. With
$e_0 = \beta_0 - \beta_0^*(\beta)$ and accepted step fraction
$\alpha \in (0,1]$, the damped intercept error is
$$
  \beta_0^+ - \beta_0^*(\beta) = (1 - \alpha)e_0 + \alpha r_0.
$$ {#eq-damped-newton-error}
The quadratic error bound applies when the full step is accepted. For
$\alpha < 1$, sufficient decrease alone does not remove the first-order term
$(1 - \alpha)e_0$.
...
To follow the correction in @eq-grad-residual through successive passes, we
study a local quadratic model. For the lasso, freeze the Hessian at the optimum
and restrict the coefficients to the active set $A$ with their signs fixed.
Assume this restricted Hessian is positive definite and define the collective
coupling
$$
...
This is a local explanation, not a global iteration-complexity theorem. A
coordinate sweep retains coefficient error modes that the exact block solve
eliminates. [Supplement S1.1](#sec-s-rate) gives the block calculation and
validates the full frozen coordinate-sweep model against nonlinear convergence
rates. The centering experiment there varies coupling while holding the
intercept curvature fixed, exposing the transition to the $L_0/H_{00}$ plateau.
...
classes, while StatLog Shuttle has $n = 58\,000$, $p = 9$, and seven classes. We
set $\lambda = 0.05\lambda_{\max}$ in both problems, where $\lambda_{\max}$ is
the smallest penalty for which all fitted slopes are zero.

On Yeoh2002, the gradient strategy needs roughly four times as many passes as
...
In @fig-irls-comparison, we compare direct CD and IRLS on a synthetic logistic
problem. Throughout the synthetic experiments, $\mu_0$ denotes the baseline mean
response at $x = 0$---a positive-class probability for binomial models and a
rate for Poisson models---and $s$ is the number of nonzero slopes in each
generated coefficient vector. Once the slopes enter the linear predictor,
$\mu_0$ need not equal the empirical response mean.
...
$\mu_0 = 0.99$, the gradient strategy needs about fifteen times as many passes
as Newton to reach $10^{-8}$ in our experiment, whereas the Newton and exact
...
by the reciprocal intercept-only curvature fraction
$[4 \pi_+ (1 - \pi_+)]^{-1}$. The table lists the curvature fraction itself,
$4 \pi_+ (1 - \pi_+)$. The six problems span a ninefold range in this quantity
and include sparse and dense designs in both dimensional regimes.
...
datasets have small intercept-only curvature fractions. Their large slowdowns
across different designs support the response-imbalance mechanism, although the
fitted curvature and collective coupling determine the local rate.
...
suboptimality bound rather than wall-clock to separate the effect of curvature
mismatch from BLAS and memory effects.
...
about eight at $\mu_0 = 0.99$. The corresponding fitted curvature ratio
$L_0/H_{00}(\hat\eta)$ rises from about three to twelve. This ratio gives the
strong-coupling limit in @eq-frozen-block-rate; it need not equal the pass-count
ratio at a finite tolerance when other convergence modes contribute.
...
The two heatmaps support the curvature-mismatch mechanism across penalties: the
empirical ratio generally grows as the intercept update becomes more
conservative ($H_{00}/L_0 \to 0$). Both curvature and coupling change across
this grid. The centering sweep of @fig-rho-centering isolates coupling by
holding $H_{00}/L_0$ fixed. The frozen coordinate-sweep model in @fig-rho-frozen
accounts for the transition from similar rates to the $L_0/H_{00}$ plateau as
the coupled mode becomes slowest.
...
### S1.1 Frozen quadratic rates and validation {#sec-s-rate .unnumbered}

The Schur analysis describes how an intercept update changes the next
coefficient gradient. To study convergence over repeated passes, we also need
the order of the updates and the errors they leave behind. We specialize to the
$\ell_1$ penalty used in the experiments, freeze the Hessian at the optimum,
restrict the coefficients to the active set $A$, and translate the optimum to
zero. Once the active signs are fixed, the penalty is locally affine and
stationarity cancels the linear terms. For the intercept and active coefficient
error vector $e$, the quadratic model is $Q(e) = \tfrac12 e^\top H e$, where
$$
  H = \begin{bmatrix}
        H_{00} & H_{0A} \\
        H_{A0} & H_{AA}
      \end{bmatrix}.
$$
The iterates move while this Hessian stays fixed. Assume $H$ is positive
definite. The collective coupling between the intercept and the active
coefficient block is $\kappa$, restating @eq-collective-coupling:
$$
  \kappa = \frac{H_{0A}H_{AA}^{-1}H_{A0}}{H_{00}} \in [0, 1).
$$
The inverse coefficient Hessian accounts for joint coefficient adjustments, so
many individually weak intercept--coefficient couplings can combine. Eliminating
the coefficient block gives the Schur complement
$H_{00} - H_{0A}H_{AA}^{-1}H_{A0} = H_{00}(1 - \kappa)$, the intercept curvature
after the coefficients have adjusted.

::: {.proposition #prp-frozen-block-rate}
*(Frozen two-block rate.)* Let $q = H_{00}/L_0 \in (0,1]$. On the frozen
quadratic, suppose $H$ is positive definite and each pass first minimizes over
the active coefficient block and then updates the intercept. The Newton and
gradient strategies contract the intercept error by
$$
  r_{\mathrm{N}} = \kappa, \qquad r_{\mathrm{G}} = 1 - q(1 - \kappa),
$$
respectively. Exact coefficient-block minimization erases the incoming
coefficient error, so the only potentially nonzero eigenvalue of each pass is
its intercept contraction factor. For fixed $0 < \kappa < 1$ and nonzero initial
intercept error, let $T_{\mathrm{G}}(\varepsilon)$ and
$T_{\mathrm{N}}(\varepsilon)$ count passes to reduce its magnitude by a factor
$\varepsilon$. Then
$$
  \lim_{\varepsilon \to 0} \frac{T_{\mathrm{G}}(\varepsilon)}{T_{\mathrm{N}}(\varepsilon)}
  = \frac{\log \kappa}{\log \left( 1 - q(1 - \kappa) \right)}
  \longrightarrow \frac{1}{q}
  = \frac{L_0}{H_{00}} \quad \text{as } \kappa \to 1.
$$ {#eq-frozen-block-rate}
For fixed $q < 1$, this asymptotic ratio diverges as $\kappa \to 0$. At a fixed
tolerance, however, Newton needs only one pass once $\kappa \leq \varepsilon$,
and the gradient pass count stays finite. These are different orders of limits.
At $\kappa = 0$, Newton solves the decoupled quadratic in one pass. When
$q = 1$, the two strategies coincide for every $\kappa$.
:::
...
We test the role of coupling by holding $\mu_0$ fixed and shifting the feature
means. For the horizontal axis of @fig-rho-centering, we summarize the
coordinate-wise couplings by their curvature-weighted mean,
$$
  \bar\rho^2 = \frac{\sum_{j=1}^p H_{jj}\rho_{0j}^2}{\sum_{j=1}^p H_{jj}}.
$$
This descriptive measure records coupling to individual coefficients; $\kappa$
also accounts for their joint adjustment through $H_{AA}^{-1}$.

On the synthetic logistic design with `means = :random`, each column carries an
$O(1)$ mean. A centering fraction $\alpha$ interpolates between the standardized
and uncentered designs,
$$
...
In @fig-rho-centering, $T_{\mathrm{G}}$ and $T_{\mathrm{N}}$ count passes to the
shared relative suboptimality bound $10^{-6}$. Their empirical ratio rises
sharply as coupling increases from near zero and then settles near the
strong-coupling limit $L_0/H_{00}$ in @eq-frozen-block-rate. To account for both
the transition and the plateau, we retain the coefficient error modes of the
actual coordinate sweep in the calculation below.

```{julia}
#| label: fig-rho-centering
#| fig-cap: Centering sweep at fixed $H_{00}/L_0$. The panels show empirical
#|   $T_{\mathrm{G}}/T_{\mathrm{N}}$ to the relative suboptimality bound
#|   $10^{-6}$; the dashed line marks the strong-coupling limit $L_0/H_{00}$
#|   from @eq-frozen-block-rate. The centering fraction $\alpha$ in
#|   @eq-centering-sweep moves the weighted $\bar\rho^2$ from standardized to
#|   uncentered designs. Here $n = 500$, $p = 1000$, $s = 10$, $\lambda =
#|   0.05\lambda_{\max}$, and the coordinate ordering is permuted.
...
spec = data(df) * mapping(:barρ2, :empirical_ratio, col = :μ0_label)

# The limit is constant within each panel because centering preserves H_00.
# Labels belong on the axis so the reference layer cannot replace them.
spec_ref = data(df) *
...
The centering experiment uses one permuted coordinate sweep per pass, which
retains coefficient error modes absent from the proposition's exact block solve.
Let $C_\pi$ be the frozen linear map for a coefficient sweep in order $\pi$, and
let
$$
  E_{\mathrm{N}} = I - \frac{1}{H_{00}}e_0e_0^\top H,
  \qquad
  E_{\mathrm{G}} = I - \frac{1}{L_0}e_0e_0^\top H = (1 - q)I + qE_{\mathrm{N}}.
$$
...
The curvature mismatch also appears in @sec-production-solvers. In glmnet's
`modified.Newton` mode and biglasso's `MM` mode, the first intercept correction
at each linearization uses the global $L_0 = 1/4$ (@sec-irls). The roughly
fivefold and tenfold slowdowns at $\mu_0 = 0.99$ in @fig-real-glmnet and
@fig-real-biglasso are consistent with this conservative correction, although
changing the weight scheme also changes the coefficient updates. skglm (version
0.5) applies the gradient strategy directly to the original loss. Its slow
convergence from suboptimality $\approx 10^{-1}$ to $\approx 10^{-5}$ in
@fig-real-skglm accompanies the intercept's slow climb toward its fitted value
at step size $1/L_0$.
...
$$
  H_{00} = D - \bar p\bar p^\top,
  \qquad
  H_{00}^{-1} = D^{-1} + \frac{1}{\bar p_K}\mathbf{1}_q\mathbf{1}_q^\top.
...
In the synthetic experiments, $\pi_k$ denotes the baseline probability used to
set $\beta_{0k} = \log(\pi_k/\pi_K)$ in the data generator. The feature effects
then change the probabilities through the softmax. Thus $\pi_k$ is the
probability at zero feature contribution, not a marginal prevalence. The feature
means are themselves seeded random draws. Both these means and the feature
amplitude affect the realized class counts. We reserve $\bar p_k$ for the
average fitted probability in the Rayleigh bounds above.

```{julia}
#| label: fig-multinomial-imbalance
#| fig-cap: Imbalanced multinomial logistic regression. The panels show a
#|   relative suboptimality bound versus time for $K = 5$, baseline
...
The realized counts in @tbl-multinomial-counts differ substantially from the
baseline probabilities. At intercept stationarity, $\bar p_k = n_k/n$, so the
observed counts give the relevant bounds, reported here to three significant
digits. For class 4, the upper bounds in @eq-multinomial-free-rayleigh are
$0.0328$, $0.0475$, and $0.145$ across the three seeds. For the reference class,
@eq-multinomial-reference-rayleigh gives $0.00394$, $0.0141$, and $0.0119$.
Dividing the latter by the Böhning majorant $L_0 = 1/2$ bounds the smallest
eigenvalue of $Q$ by $0.00787$, $0.0282$, and $0.0238$, respectively. These
realized prevalences therefore still imply a strongly attenuated gradient
correction in at least one intercept eigenmode near stationarity
(@eq-grad-residual-matrix).

  | Seed | Class 1 | Class 2 | Class 3 | Class 4 | Class 5 (reference) |
  | ---: | ------: | ------: | ------: | ------: | ------------------: |
  |    1 |     417 |      53 |       5 |      17 |                   8 |
  |    2 |     374 |       7 |      64 |      25 |                  30 |
  |    3 |     142 |     143 |     102 |      88 |                  25 |

  : Realized class counts for @fig-multinomial-imbalance, with $n=500$ and
    baseline probabilities $(0.7,0.1,0.1,0.05,0.05)$. The seeded input
    reconstruction in `experiments/sim-multinomial-prevalence.jl` reports counts
    for all sweep cells in `results/sim-multinomial-prevalence.csv`.
    {#tbl-multinomial-counts}

The gradient strategy reaches the same optimum but takes roughly an order of
magnitude longer. Across the three seeds, Newton and exact converge within
roughly $50$--$100$ passes, while the gradient strategy takes several hundred
and, in the slowest run, exhausts the thousand-pass budget.

The Newton and exact strategies follow the same visible curve. The
$(K - 1)$-block Newton solve uses the inverse of the current intercept Hessian,
including the off-diagonal coupling $H_{00,kl} = -(1/n)\sum_i p_{ik}p_{il}$. The
explicit inverse in @eq-multinomial-intercept-inverse describes the special case
of probabilities constant across observations. In the fitted model, the full
Hessian solve likewise rescales the low-curvature eigenmodes that the gradient
strategy attenuates. The Armijo guard supplies descent during the cold-start
transient; full-step local accuracy applies when damping is inactive
(@eq-damped-newton-error).

We extend the test to a two-dimensional grid by shrinking the reference-class
baseline probability $\pi_K$ to $0.005$ and increasing the feature amplitude to
four times the baseline value. The four corners appear in
@fig-multinomial-sweep; across the full sweep (@fig-multinomial-sweep-full), the
Newton and exact trajectories overlap throughout. They reach tight certificates
in all but the two most extreme cells, while the gradient strategy generally
slows as $\pi_K$ shrinks and amplitude grows. In those two extreme cells
($\pi_K = 0.005$ and amplitude at least $1.5$), no strategy obtains a useful
median certificate within 1,000 passes. Those cells do not isolate the intercept
update and therefore do not support the comparison. Each has two seeds with no
reference-class observations, as detailed in Supplement S2.2.
...
#|   reference-class baseline probability $\pi_K \in \{0.05, 0.005\}$ and
#|   amplitude $\in \{0.5, 2.0\}$ grid on the same $K = 5$, $n = 500$, $p =
#|   200$, $s = 5$, $\lambda = 0.05\lambda_{\max}$, $\rho = 0.3$ design as
...
agg.p_K_str = [L"$\pi_K = %$(v)$" for v in agg.p_K]
agg.amp_str = "amp = " .* string.(agg.amplitude)

# The four corners show the endpoints; the full sweep appears in the appendix.
corners = filter(
    r -> r.p_K_str in (L"$\pi_K = 0.05$", L"$\pi_K = 0.005$") &&
...
spec = data(corners) *
    mapping(
        ...
        :relgaps => "Relative suboptimality",
        color = :strategy,
        row = :p_K_str => sorter(L"$\pi_K = 0.05$", L"$\pi_K = 0.005$"),
...
$\lambda = 0.05\lambda_{\max}$. The reference-class baseline probability $\pi_K$
ranges from $0.05$ to $0.005$, and the feature amplitude ranges from $0.5$ to
$2.0$. Each curve is the median relative suboptimality bound over three seeds.
This varies baseline log odds and signal amplitude; the realized prevalences
also change with amplitude and seed.
...
#|   grid of reference-class baseline probability $\pi_K \in \{0.05, 0.02, 0.01,
#|   0.005\}$ and feature amplitude $\in \{0.5, 1.0, 1.5, 2.0\}$. Curves are
#|   medians over three seeds for $K = 5$, $n = 500$, $p = 200$, $s = 5$, $\rho
#|   = 0.3$, and $\lambda = 0.05\lambda_{\max}$; early-terminated trajectories
#|   are carried forward at their terminal values.
...
Across the grid, the gradient strategy generally stalls more severely as $\pi_K$
decreases and feature amplitude grows; Newton and exact remain nearly
indistinguishable. In the two cells with $\pi_K = 0.005$ and amplitude at least
$1.5$, however, their shared median bound remains far from zero. The realized
reference-class counts for seeds 1--3 are $(0,8,0)$ at amplitude $1.5$ and
$(0,12,0)$ at amplitude $2.0$. When the reference class is absent, increasing
all free-class intercepts together strictly decreases the loss at every finite
iterate, without changing the penalty. There is therefore no finite intercept
minimizer for those replicates, and the positive-curvature local analysis at a
finite optimum does not apply. The failed median certificates in these cells
cannot identify coefficient convergence or near-separation as the cause.

The class-count cache also flags absent-class replicates elsewhere in the sweep.
An objective certificate can still bound distance to the infimum in these
boundary cases, but it does not imply convergence to finite intercepts. We
retain their trajectories to show this limitation. The curvature analysis at a
finite optimum applies to replicates with every class observed. The solvers use
the same seeded data within each problem, although the grid does not hold
marginal prevalence fixed as amplitude varies. Where the gradient strategy
reaches the $10^{-6}$ threshold, it generally needs at least an order of
magnitude more passes than Newton. In most of the remaining certified cells, it
misses that threshold within the $1\,000$-pass budget even though Newton reaches
it.
...
Every reported result for the exact strategy met the corresponding
conditional-solve criterion.
...
diagnostic $t$, we set
$$
  \underline D = \max_{s,t} D_{s,t}, \qquad \overline P = \min_{s,t} P_{s,t},
  \qquad
  B_{s,t} = \frac{P_{s,t} - \underline D}{|\overline P|}.
